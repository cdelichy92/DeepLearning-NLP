...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      embeddings = tf.Variable(self.wv, name="Embedding", dtype = tf.float32)
      window = tf.reshape(tf.nn.embedding_lookup(embeddings, self.input_placeholder), 
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope("Layer"):  
        ...
        self.b1 = tf.get_variable("bias", (self.config.hidden_size,), 
                                  initializer=tf.constant_initializer(0.0))
        
    h = tf.tanh(tf.matmul(tf.cast(window, dtype=tf.float32), self.W) + self.b1)
    h = tf.nn.dropout(h, self.dropout_placeholder)
    
    with tf.variable_scope("Softmax"):
        self.U = tf.get_variable("weights", (self.config.hidden_size, self.config.label_size), 
                                 initializer=xavier_weight_init())
        self.b2 = tf.get_variable("bias", (self.config.label_size,), 
                                  initializer=tf.constant_initializer(0.0))
  ...
  def add_loss_op(self, y):
    ...
    """
    #W = tf.get_variable("Layer/weights:0")
    #U = tf.get_variable("Softmax/weights:0")
   
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder))
    loss += 0.5 * self.config.l2 * (tf.nn.l2_loss(self.W) + tf.nn.l2_loss(self.U))   
