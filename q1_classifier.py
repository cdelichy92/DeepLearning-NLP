...
class SoftmaxModel(Model):
  ...
  def create_feed_dict(self, input_batch, label_batch):
    ...
    feed_dict[self.input_placeholder] = np.reshape(input_batch, (self.config.batch_size,self.config.n_features))
    
    if not label_batch==None:
        feed_dict[self.labels_placeholder] = np.reshape(label_batch, (self.config.batch_size,self.config.n_classes))
  ...
  def add_model(self, input_data):
    ...
    with tf.variable_scope("parameters"):
        ...
        b = tf.get_variable("bias", (self.config.batch_size, self.config.n_classes), initializer=tf.constant_initializer(0.0))
