...
class RNN_Model():
    ...
    def add_model_vars(self):
        ...
        with tf.variable_scope('Projection'):
            ...
            bs = tf.get_variable('bs', [self.config.label_size])
    ...
    def add_model(self, node):
        ...
        if node.isLeaf:
            ...
            hl, hr = node_tensors[node.left], node_tensors[node.right]
            h_concat = tf.concat(1, [hl, hr])
            curr_node_tensor = tf.nn.relu(tf.matmul(h_concat, W1) + b1)
    ...
    def loss(self, logits, labels):
        ...
        loss += self.config.l2 * (tf.nn.l2_loss(U, name=None) + 
                                  tf.nn.l2_loss(W1, name=None) )
