...
#      L = tf.Variable(tf.random_normal([len(self.vocab), self.config.embed_size]),
#                      name="Embedding",
#                      dtype = tf.float32)
        ...
        inputs_split = tf.split(dim, self.config.num_steps, embed)
      
        inputs = []
                            
        for wv in inputs_split:
            wv = tf.squeeze(wv)
            if self.config.batch_size == 1:
                wv = tf.reshape(wv, [1,-1])
            inputs.append(wv)
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope("Score"):  
        ...
        b2 = tf.get_variable("bias", (len(self.vocab),))

    #U = tf.Variable(tf.random_normal([self.config.hidden_size, len(self.vocab)]), dtype = tf.float32)
    #b2 = tf.Variable(tf.zeros([len(self.vocab),]))
    outputs = []
    
    for output in rnn_outputs:
        score = tf.matmul(output, U) + b2
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("HiddenLayer"):  
            I = tf.get_variable("weights_x", 
                                (self.config.embed_size, self.config.hidden_size))
            H = tf.get_variable("weights_h", 
    ...
    for x in inputs:
        ...
        with tf.variable_scope("HiddenLayer", reuse=True):  
            I = tf.get_variable("weights_x", 
                                (self.config.embed_size, self.config.hidden_size))
            H = tf.get_variable("weights_h", 
                                (self.config.hidden_size, self.config.hidden_size))
        ...
        h = tf.sigmoid(tf.matmul(h, H) + tf.matmul(x, I) + b1)
        
        output = tf.nn.dropout(h, self.dropout_placeholder)
...
def generate_text(session, model, config, starting_text='<eos>',
  ...
  for i in xrange(stop_length):
    ...
    state, y_pred = session.run([model.final_state, model.predictions[0]], 
                                feed_dict=feed)
